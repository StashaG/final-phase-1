$(()=> {
  ...
  $('form').on("submit", function(e){
    e.preventDefault(); 
      ...
      let recipe = fetch(`https://api.spoonacular.com/recipes/findByIngredients?ingredients=${$('.search-bar').val()}&number=2&apiKey=45f77eeec63346cdb3c6db8bfd6183e3`)
          ...
          .then(data => {
            ...
            function recipesuggestion(fridgeFood) {
                ...
                recipeFind = fridgeFood.map(data => {
                    ...
                    <div class="card-body">
                      <p class="card-text" id="recipe">${data.usedIngredients}</p>
                    </div>`
                });
                $('.recipeOp').html(recipeFind.join(' '))
      
        };
        recipesuggestion(data.Search);
